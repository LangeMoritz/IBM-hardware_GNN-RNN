...
class MWPMDecoder:
    ...
    def _load_job_data(self) -> None:
        ...
        self.detections_list, self.flips_list, = self.sampler.load_jobdata(verbose=True)
    ...
    def decode(self) -> float:
        ...
        if self.load_distance == self.distance:
            self.detections, self.flips, = self.detections_list, self.flips_list
            self.train_val_split()
            self._get_edges()
            pdet_mean = self.val_detections.mean()
            logical_accuracy, logical_accuracy_err = self._evaluate_predictions()
            return np.array(logical_accuracy), np.array(logical_accuracy_err), np.array(pdet_mean)

        else:
            logical_accuracy_list, logical_accuracy_err_list, pdet_mean_list = [], [], []
            for detections, flips in zip(self.detections_list, self.flips_list):
                self.detections, self.flips = detections, flips
                self.train_val_split()
                self._get_edges()
                logical_accuracy, logical_accuracy_err = self._evaluate_predictions()
                logical_accuracy_list.append(logical_accuracy)
                logical_accuracy_err_list.append(logical_accuracy_err)
                pdet_mean_list.append(self.val_detections.mean())
            return np.array(logical_accuracy_list), np.array(logical_accuracy_err_list), np.array(pdet_mean_list)
